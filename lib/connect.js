const { useMultiFileAuthState, Browsers, fetchLatestBaileysVersion, makeCacheableSignalKeyStore } = require('@whiskeysockets/baileys');
...
async function connectionLogic() {
  ...
  if (fs.existsSync(credsFile)) {
    console.log('Using existing creds data ☁️');
  } else {
    try {
  ...
  // Read the creds.json file after it's loaded
  const credsData = fs.readFileSync(credsFile, 'utf8');
  const creds = JSON.parse(credsData);

  // Get the bot's number (me.id)
  const botNumber = creds.me.id; // This will give you the bot's number (e.g., 2348073765008:14@s.whatsapp.net)
